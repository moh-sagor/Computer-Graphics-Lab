...
void display(void)
{
 ...
 //The function called when the window size changes
void ChangeSize(GLsizei w, GLsizei h)
{
    ...
    if (h == 0) {
        h = 1;
         ...
         //Define the size of the viewport with the same width and height
    glViewport(0, 0, w, h);
    int half = 200;
         //Reset the coordinate system to reset the projection transformation
    glMatrixMode(GL_PROJECTION);
         //Move the origin of the current user coordinate system to the center of the screen
    glLoadIdentity();
         //Define the orthogonal view field
    if (w < h) {
                 //If the height is greater than the width, the height viewing angle will be enlarged and the graphic display will be centered
        glOrtho(-half, half, -half * h / w, half * h / w, -half, half);
    }
    else {
                 //If the width is greater than the height, the width viewing angle will be enlarged and the graphic display will be centered
...
int main(int argc, char* argv[]) {
    ...
    glutInitWindowSize(400, 400);
         glutCreateWindow("wireframe robot");
